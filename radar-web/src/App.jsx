...
// LISTADO COMPLETO DE LAS 49 CLÍNICAS (SEGÚN TU EXCEL)
const clinicsData = [
  { id: 0, name: "CE01 T.S (BASE)", lat: 6.2308, lng: -75.5905, zone: "CENTRAL" },
  // SUR ALTO
  { id: 1, name: "HOSP. SAN VICENTE - CALDAS", lat: 6.0911, lng: -75.6358, zone: "SUR ALTO" },
  ...
  { id: 14, name: "MEDELLIN SEDE POBLADO", lat: 6.2001, lng: -75.5661, zone: "SUR BAJO" },
  ...
  { id: 19, name: "LAS AMERICAS", lat: 6.2215, lng: -75.5942, zone: "SUR BAJO" },
  { id: 20, name: "MEDELLIN SEDE OCCIDENTE", lat: 6.2405, lng: -75.5895, zone: "SUR BAJO" },
  { id: 21, name: "INSTITUTO DEL DOLOR", lat: 6.2410, lng: -75.5710, zone: "SUR BAJO" },
  { id: 22, name: "HOSPITAL GENERAL", lat: 6.2371, lng: -75.5694, zone: "SUR BAJO" },
  ...
  { id: 28, name: "SAGRADO CORAZON", lat: 6.2422, lng: -75.5563, zone: "CENTRO" },
  { id: 29, name: "ROSARIO CENTRO", lat: 6.2482, lng: -75.5615, zone: "CENTRO" },
  { id: 30, name: "SAN VICENTE MEDELLIN", lat: 6.2625, lng: -75.5655, zone: "CENTRO" },
  { id: 31, name: "LEON XIII", lat: 6.2612, lng: -75.5638, zone: "CENTRO" },
  ...
  { id: 35, name: "NEUROLOGICO", lat: 6.2618, lng: -75.5635, zone: "CENTRO" },
  ...
  // ORIENTE
  { id: 44, name: "SAN JUAN DE DIOS RIONEGRO", lat: 6.1512, lng: -75.3735, zone: "ORIENTE" },
...
export default function App() {
  ...
  // Evita el bucle infinito al pedir la ruta
  const directionsCallback = useCallback((res) => {
    if (res !== null && res.status === 'OK' && !directions) {
      setDirections(res);
    }
  }, [directions]);
  ...
  if (!isLoaded) return <div className="h-screen bg-[#020617] flex items-center justify-center text-blue-400 font-black italic">CONECTANDO SATÉLITE...</div>;

  return (
    <div className="flex h-screen bg-[#020617] text-slate-100 font-sans overflow-hidden">
      
      {/* SIDEBAR CON TODAS LAS CLÍNICAS */}
      <aside className={`transition-all duration-300 z-30 bg-[#0f172a]/95 backdrop-blur-md border-r border-blue-500/20 shadow-2xl flex flex-col ${sidebarOpen ? 'w-72' : 'w-0 -translate-x-full'}`}>
        <div className="p-4 w-72 flex flex-col h-full overflow-hidden">
          <header className="mb-4">
            ...
            <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mt-1">49 Clínicas Monitoreadas</p>
          </header>

          <div className="space-y-4 flex-1 overflow-y-auto pr-1 custom-scrollbar">
            <div className="bg-slate-900/80 p-3 rounded-2xl border border-blue-500/20">
              <label className="text-[9px] font-black text-blue-400 uppercase">Desde</label>
              ...
              <label className="text-[9px] font-black text-green-400 uppercase block">Hacia</label>
            ...
            <div className="space-y-1.5">
              <input type="text" placeholder="🔍 Filtrar clínicas..." className="w-full bg-slate-900 border border-slate-800 rounded-xl py-2 px-3 text-[10px] outline-none" onChange={(e) => setSearchTerm(e.target.value)} />
              {filteredClinics.map(clinic => (
                <button
                  ...
                  className={`w-full text-left p-3 rounded-xl border transition-all ${
                    destination?.id === clinic.id ? 'bg-blue-600 border-blue-400' : 'bg-slate-900/40 border-slate-800 hover:border-blue-700'
                  }`}
                >
                  <p className="text-[9px] font-bold uppercase truncate">{clinic.name}</p>
          ...
          {routeSummary && (
            <div className="mt-4 p-4 bg-blue-600/10 border border-blue-500/30 rounded-2xl">
              <div className="grid grid-cols-2 gap-2 mb-3">
                <div className="text-center">
                  <p className="text-[8px] text-slate-400 uppercase">Distancia</p>
                  <p className="text-xs font-black text-blue-400">{routeSummary.distance}</p>
                </div>
                <div className="text-center border-l border-slate-700">
                  <p className="text-[8px] text-slate-400 uppercase italic">Con Tráfico Real</p>
                  <p className="text-xs font-black text-green-400">{routeSummary.time}</p>
                </div>
              </div>
              <button id="copyBtn" onClick={copyForWhatsApp} className="w-full py-3 bg-green-600 hover:bg-green-700 rounded-xl text-[9px] font-black uppercase tracking-widest">COPIAR REPORTE</button>
            </div>
          )}
        </div>
      </aside>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .marker-label { background: rgba(15, 23, 42, 0.8); padding: 1px 4px; border-radius: 4px; pointer-events: none; }
      `}} />
    </div>
  );
}
