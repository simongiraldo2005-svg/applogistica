...
// LISTADO DE LAS 49 CLÍNICAS (SEGÚN TU EXCEL)
const clinicsData = [
...
export default function App() {
  ...
  const routeSummary = useMemo(() => {
    ...
    // TIEMPO BASE: Lo que se demora sin tráfico
    const durationBase = leg.duration.value; // en segundos
    // TIEMPO REAL: Lo que se demora con el tráfico de este instante
    const durationTraffic = leg.duration_in_traffic ? leg.duration_in_traffic.value : durationBase;
    
    // CÁLCULO DE DEMORA POR TACO
    const trafficDelaySec = durationTraffic - durationBase;
    const trafficDelayMin = Math.max(0, Math.floor(trafficDelaySec / 60));

    return {
      distance: leg.distance.text,
      timeNormal: leg.duration.text,
      timeTrafficDelay: trafficDelayMin > 0 ? `+${trafficDelayMin} min de taco` : "Vía Fluida",
  ...
  if (!isLoaded) return <div className="h-screen bg-[#020617] flex items-center justify-center text-blue-400 font-black italic">CONECTANDO SISTEMA FLEET...</div>;
  ...
  return (
    <div className="flex h-screen bg-[#020617] text-slate-100 font-sans overflow-hidden">
      ...
      <aside className={`transition-all duration-300 z-30 bg-[#0f172a]/95 backdrop-blur-md border-r border-blue-500/20 shadow-2xl flex flex-col ${sidebarOpen ? 'w-80' : 'w-0 -translate-x-full'}`}>
        <div className="p-4 w-80 flex flex-col h-full overflow-hidden">
          <header className="mb-4">
            ...
            <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mt-1">Sincronización Logística Total</p>
          </header>
          ...
          <div className="space-y-4 flex-1 overflow-y-auto pr-1 custom-scrollbar">
            ...
            <div className="space-y-1">
              <input type="text" placeholder="🔍 Filtrar clínicas..." className="w-full bg-slate-900 border border-slate-800 rounded-xl py-2 px-3 text-[10px] outline-none mb-2" onChange={(e) => setSearchTerm(e.target.value)} />
          ...
          {routeSummary && (
            <div className="mt-4 p-4 bg-blue-600/10 border border-blue-500/30 rounded-2xl animate-in slide-in-from-bottom-2">
              <div className="space-y-2 mb-3">
                ...
                <div className="flex justify-between text-[10px] border-b border-white/5 pb-1">
                  <span className="text-slate-400 uppercase">Tiempo en Taco:</span>
                  <span className="font-black text-orange-400">{routeSummary.timeTrafficDelay}</span>
                </div>
          ...
          {origin && destination && !directions && (
            <DirectionsService
              options={{
                ...
                travelMode: 'DRIVING',
                drivingOptions: {
                  departureTime: new Date(), // ESTO ACTIVA EL CÁLCULO DE TRÁFICO REAL
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .marker-label { background: rgba(15, 23, 42, 0.85); padding: 2px 5px; border-radius: 4px; pointer-events: none; text-shadow: 0 1px 2px black; }
      `}} />
    </div>
  );
}
